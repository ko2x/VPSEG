...
then
	...
	then
		...
		then
			tput setaf 7 ; tput setab 4 ; tput bold ; echo "" ; echo "O domain $host already exists in the file $payload" ; echo "" ; tput sgr0
			exit 1
		else
			if [[ $host != \.* ]]
			then
				tput setaf 7 ; tput setab 4 ; tput bold ; echo "" ; echo "You must add a domain starting with a period!" ; echo "example: .phreaker56.xyz" ; echo "It is not necessary to add subdomains for domains that are already in the file" ; echo "That is, there is no need to add recargawap.claro.com.br" ; echo "if the .claro.com.br domain is already in the file." ; echo ""; tput sgr0
				exit 1
			else
				...
				tput setaf 7 ; tput setab 1 ; tput bold ; echo "" ; echo "The Squid Proxy has been successfully loaded!" ; echo "" ; tput sgr0
				exit 1
			fi
		fi
	fi
